...
**Newsletter** (только фронтенд — бэкенд-эндпоинт рассылки ещё не готов).
...
- Кнопка «Разослать» открывает диалог подтверждения со сводкой (включая число
  кнопок и их тип). Отправка сейчас имитируется (мок) и завершается успехом;
  реальная отправка появится после подключения бэкенда.
