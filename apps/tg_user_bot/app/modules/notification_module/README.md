...
5. Если указан `file_id` — файл скачивается из бэкенда один раз; при первой отправке используются байты, для остальных получателей переиспользуется Telegram file_id из ответа.
