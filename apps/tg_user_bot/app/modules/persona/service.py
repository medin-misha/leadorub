"""
Воронка DMCAGuardian: стартовая цепочка выдачи материалов.
...
from app.modules.persona.keyboards import get_contact_keyboard
from app.modules.persona.messages import get_messages

logger = logging.getLogger(__name__)

# Именованное состояние воронки в backend (`user_stats.state`).
PERSONA_START_STATE = "persona_start"

GUIDE_FILENAME = "DMCAGuardian — как убрать свои фото и видео из сети.pdf"
...
async def run_start_funnel(message: Message) -> None:
    """Проводит пользователя по стартовой цепочке: текст → PDF → видео с кнопками."""
    ...
    await _mark_persona_start(message)


async def _send_guide(message: Message) -> None:
    """Отправляет PDF-гайд; отсутствие файла логируется, но не роняет цепочку."""

    guide_path = settings.persona_guide_path
    if not guide_path.is_file():
        logger.error("Persona guide is missing: %s", guide_path)
        return
...
async def _send_video(message: Message) -> None:
    ...
    video_path = settings.persona_video_path
    keyboard = get_contact_keyboard()

    if not video_path.is_file():
        logger.error("Persona video is missing: %s", video_path)
...
async def _mark_persona_start(message: Message) -> None:
