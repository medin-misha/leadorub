...
Backend Leadorub — центральный API для Telegram-платформы сбора лидов. Он хранит
пользователей, профили, заявки, чат поддержки и файлы, обслуживает админ-панель и
координирует доставку событий через RabbitMQ.

### Ключевой функционал

- административная JWT-аутентификация и управление администраторами;
- регистрация Telegram-пользователей и сбор статистики;
- заявки из бота и Telegram Mini App с проверкой подписанного `initData`;
- чат поддержки с файловыми вложениями;
- фильтрованные рассылки с файлами и клавиатурами;
- S3-хранилище на MinIO;
- надёжная публикация DB-зависимых событий через transactional outbox.

### Окружения и ссылки

- **Локально**: `http://127.0.0.1:8000`.
- **Production**: API доступен через edge Caddy на origin админ-панели; прямой
  порт backend привязан к loopback.

# Документация backend

Backend построен на модульной архитектуре FastAPI и PostgreSQL. Подробный
контекст для Claude Code находится в `.claude/CLAUDE.md`, а контракты отдельных
доменов — в локальных `AGENTS.md`/`CLAUDE.md` и русских `README.md` модулей.
...
- **Обобщенный CRUD-сервис** (`CRUD` в `services/crud.py`): содержит методы асинхронного создания (`create`, `bulk_create`, `get_or_create` для атомарной бесконфликтной вставки), чтения (`get` с поддержкой пагинации, сортировки, фильтрации и полнотекстового поиска; `count` — подсчёт записей под фильтром; `get_column` — выборка значений одной колонки под тем же фильтром), обновления (`patch`) и удаления (`delete`).
...
- **API эндпоинты**:
  - `POST /api/files/` — загрузка файлов через multipart/form-data. Фактический
    размер проверяется до S3; лимит задаётся `file_upload_max_size_bytes`.
...
- **Transactional outbox**: DB-зависимое событие сохраняется через
  `enqueue_outbox_message()` в транзакции бизнес-операции. Фоновый publisher
  публикует только после commit, использует lease, стабильный `message_id` и
  повтор с exponential backoff.
