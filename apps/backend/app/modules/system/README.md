...
- `count(...)` - считает записи под фильтром `search`/`field` (без пагинации);
- `get_column(...)` - возвращает значения одной колонки для всех записей под фильтром (без пагинации);
- `patch(...)` - частично обновляет существующую запись;
- `delete(...)` - удаляет запись по `id`.

`count(...)` и `get_column(...)` переиспользуют ту же логику фильтрации, что и `get(...)`, через общий хелпер `_apply_search(...)` — так фильтр по `search`/`field` описан в одном месте. Типовое применение: посчитать аудиторию под фильтром и затем выбрать список значений (например, `telegram_id` получателей рассылки).
