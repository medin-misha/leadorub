...
1. **Создание заявки от пользователя:**
   ...
   * Client MiniApp отправляет заявку через `POST /api/requisitions/public` и передаёт исходную строку Telegram WebApp `initData` в заголовке `X-Telegram-Init-Data`.
   * Backend проверяет HMAC-подпись, `auth_date` и срок жизни данных, после чего извлекает `telegram_id` из подписанного поля `user`. Значение ID из тела запроса не принимается.
   * Маршрут не использует `X-Service-Token`: секрет server-to-server не передаётся публичному Caddy. Остальные `/api/*` на домене MiniApp возвращают `404`.
